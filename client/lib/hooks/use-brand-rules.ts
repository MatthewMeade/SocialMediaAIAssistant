...
export function useBrandRules(calendarId: string) {
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      return apiGet<BrandRule[]>(`/api/brand-voice?calendarId=${calendarId}`)
    },
    enabled: !!calendarId,
  })
  ...
  const createMutation = useMutation({
    mutationFn: async (rule: Omit<BrandRule, "id">) => {
      return apiPost<BrandRule>("/api/brand-voice", rule)
    },
  ...
  const updateMutation = useMutation({
    mutationFn: async (rule: BrandRule) => {
      return apiPut<BrandRule>("/api/brand-voice", rule)
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: async (ruleId: string) => {
      return apiDelete(`/api/brand-voice?id=${ruleId}`)
    },
